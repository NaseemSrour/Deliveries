...
import database.db_firestore_controller as db_firestore_controller
...
class DBItem():
    ...
    def __init__(self, name: str, desc: str, image: bytes, price: int, business_id: int):
        self.name = name  # ID in Firestore Database
        self.desc = desc
        self.image = image
        self.price = price
    ...
    def delete_item(self):
        ...
        db_mysql_controller.execute_query(delete_statement, data)  # Returns None
...
def get_mysql_item(item_id: int) -> DBItem:
    if (type(
            item_id) != int):  # MAYBE WE ALSO PREVENT USER FROM INPUTING ANYTHING OTHER THAN A NUMBER (in the  web forms + in the HTTP requests).
    ...
    results = db_mysql_controller.execute_query(select_query,
                                                data)  # Returns a list of tuples; in this case, should be one tuple.
    ...
    if (results is not None and len(results) > 0):
        ...
        retrieved_item = DBItem(res_item[1], res_item[2], res_item[3], res_item[4], res_item[5],
                                res_item[0])  # Indexes are according to the SELECT statement above.
...
# ---------------------
# Returns: DBItem instance, or None on failure.
def get_item(item_name: str) -> DBItem:
    if type(item_name) != str:  # MAYBE WE ALSO PREVENT USER FROM INPUTING ANYTHING OTHER THAN A NUMBER (in the  web forms + in the HTTP requests).
        err_msg = "item_name must be string!"
        return None
    ...
    try:
        ...
        err_msg = "Connection to Cloud Firestore failed!"
        print(err_msg)
        return None

    item_ref = db.collection('items').document(item_name)
    res_item = item_ref.get()
    if res_item.exists:
        print(f'Retrieved item data: {res_item.to_dict()}')
        retrieved_item = DBItem.toObject(json.dumps(res_item.to_dict()))
        return retrieved_item
    else:
        logger.info(u'No such item ' + item_name)
        return None
...
def get_items(businessID: int) -> list[DBItem]:
    ...
    data = (businessID,)
    results = db_mysql_controller.execute_query(select_query, data)  # returns a list of tuples
